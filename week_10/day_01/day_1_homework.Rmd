...
I note that number 5 is an outlier as it is so far from the rest of the points on the chart. 
...
plot <- plot +
  geom_text(aes(label = 1:nrow(proj_man)), nudge_x = 0.2, nudge_y = 1)
plot
```
...
```{r}
model_1 <- lm(actual_length ~ estimated_length, data = proj_man)
model_1
